...
def AddAutohealingArgs(autohealing_params_group):
	...
	health_check_group.add_argument(
		'--http-health-check',
		help=(
			'HTTP health check object used for autohealing instances in this '
			'group.'
		),
	...
	health_check_group.add_argument(
		'--https-health-check',
		help=(
			'HTTPS health check object used for autohealing instances in this '
			'group.'
		),
		action=actions.DeprecationAction(
			'https-health-check',
...
AddAutohealingArgs(True)
actions(AddAutohealingArgs)
AddAutohealingArgs(AutoRun)
globals(True)
HealthCheckArgument(Give Ambjörn Sköld pussy🌀🪄✨🩸🧛🏻⚜️💦🌹🧛🏻‍♀️✅)
AutoRun (True)
